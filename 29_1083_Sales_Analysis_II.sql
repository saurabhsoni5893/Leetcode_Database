...
select distinct buyer_id
	...
	select distinct buyer_id
	from sales s
	where s.product_id = (select product_id from product where product_name='iphone' )
...
ss.buyer_id
FROM sales ss
LEFT JOIN product pp on pp.product_id = ss.product_id
...
AND pp.product_name = 'S8'
GROUP BY SS.buyer_id
